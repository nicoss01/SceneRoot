...
/**
 * Télécommande HDMI-CEC. Chaque action réaffiche l'affichage de mpv, seul
 * visible pendant la lecture : il occupe l'écran devant le navigateur, donc
 * l'interface web ne peut pas s'y superposer.
 */
...
function writeMpvInput(){
  ...
  const bindings=[
    ...
    'v script-message sceneroot-menu',
    'j cycle sub; show-text "Sous-titres : ${?sid==no:aucun}${!sid==no:${sub-lang} (piste ${sid})}" 3000',
    'a cycle audio; show-text "Audio : ${audio-lang} (piste ${aid})" 3000',
...
function spawnMpv(path:string,start:number){
  ...
  // La variante « copy » ramène les images, au prix de quelques copies : le
  // profil « fast » de mpv compense sur un Raspberry Pi.
...
/**
 * Tendances de la semaine selon TMDB. Sans clé, le catalogue local retombe sur
 * son classement par popularité : les titres les plus votés d'abord.
 */
async function tmdbTrending(kind:'film'|'serie',page:number,limit:number):Promise<ProviderPage>{
  const key=process.env.TMDB_API_KEY;if(!key)throw new Error('TMDB non configuré');
  const url=new URL(`https://api.themoviedb.org/3/trending/${kind==='serie'?'tv':'movie'}/week`);
  url.searchParams.set('api_key',key);url.searchParams.set('language','fr-FR');url.searchParams.set('page',String(page));
  const result=await cachedJson<{results:Record<string,unknown>[];total_pages:number}>(`catalog-trending-tmdb-${kind}-${page}`,url.toString(),6*60*60*1000);
  // Une sortie de moins d'un mois porte la pastille « Nouveau » : sans elle,
  // l'accueil n'aurait plus aucun repère de nouveauté.
  const fresh=new Date(Date.now()-30*24*60*60*1000).toISOString().slice(0,10);
  const items=result.data.results.slice(0,limit).map(item=>{const entry=normalizeTmdbCatalog(item,kind);return entry.releaseDate&&entry.releaseDate>=fresh?{...entry,badge:'new' as const}:entry});
  return{items,hasMore:page<Number(result.data.total_pages??1),source:'Tendances TMDB',cachedAt:result.cachedAt,cacheState:result.state};
}
...
let imdbSyncStop:AbortController|null=null;
...
/** Vide le catalogue local : utile après un import partiel ou pour repartir à neuf. */
app.delete('/api/catalog',async(_request,reply)=>{
  if(!catalogStore.available)return reply.code(503).send({error:'SQLite nécessite Node.js 22.5 ou supérieur'});
  imdbSyncStop?.abort();
  if(imdbSyncPromise)await imdbSyncPromise.catch(()=>{/* arrêt demandé */});
  catalogStore.reset();
  return{ok:true,total:catalogStore.count()};
});
...
app.post<{Body:{command:string;value?:number|string}}>('/api/player/control',{schema:{body:{type:'object',required:['command'],properties:{command:{type:'string',enum:['pause','play','stop','seek-forward','seek-back','audio-next','subtitle-next','seek-to','set-audio','set-subtitle','next-countdown']},value:{type:['number','string']}}}}},async(request,reply)=>{const{command,value}=request.body;let mpvCmd:unknown[]|undefined={pause:['cycle','pause'],play:['set_property','pause',false],stop:['quit'],'seek-forward':['seek',30,'relative'],'seek-back':['seek',-10,'relative'],'audio-next':['cycle','audio'],'subtitle-next':['cycle','sub']}[command];if(command==='next-countdown')mpvCmd=['script-message','sceneroot-next',String(value??-1)];else if(command==='seek-to'&&typeof value==='number')mpvCmd=['seek',value,'absolute'];else if(command==='set-audio')mpvCmd=['set_property','aid',value??'auto'];else if(command==='set-subtitle')mpvCmd=['set_property','sid',value===undefined||value==='no'?'no':value];if(!mpvCmd)return reply.code(400).send({error:'Commande inconnue'});try{await mpvCommand(mpvCmd);return{ok:true}}catch{return reply.code(503).send({error:'Lecteur mpv indisponible'})}});
...
function warmRecentCatalog(){void catalogFor('film',1,12,'trending').catch(()=>{});void catalogFor('serie',1,12,'trending').catch(()=>{})}
warmRecentCatalog();
