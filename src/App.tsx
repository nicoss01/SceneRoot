...
function DetailPage({profile}:{profile:Profile}) {
  ...
  return <div className="detail" style={{ '--a': item.palette[0], '--b': item.palette[1], backgroundImage:`linear-gradient(90deg,rgba(1,7,14,.94) 8%,rgba(1,7,14,.28)), url('${posterUrl(item.art)??'/assets/sceneroot-landscape.png'}')` } as React.CSSProperties}>
...
function PlayerPage({profile}:{profile:Profile}) {
  ...
  return <div className={`player ${chromeVisible?'':'chrome-hidden'}`} style={{ '--a': item.palette[0], '--b': item.palette[1], backgroundImage:`linear-gradient(105deg,rgba(1,7,14,.5),transparent 60%), url('${posterUrl(item.art)??'/assets/sceneroot-landscape.png'}')` } as React.CSSProperties}>
...
type AppSettings={minFreeGb:number;preferredQuality:string;preferredLanguages:string[];preferHdr:boolean;catalogSyncEnabled:boolean;tmdb:{configured:boolean;source:'environment'|'settings'|'none';maskedKey?:string};envSources:{id:string;name:string}[];sources:MaskedSource[]};
