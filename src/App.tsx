...
function SettingsPage() {
  ...
  return <><div className="welcome"><h1>Paramètres</h1><p>Configurez votre médiathèque, la lecture et l’appareil.</p></div><div className="settings-grid">
    ...
    <div className="settings-card catalog-settings"><h2><Film/>Catalogue local</h2><p><b>{(catalogStatus?.total??0).toLocaleString('fr-FR')}</b> films et séries dans SQLite.</p><Setting label="Synchronisation IMDb périodique" value={settings?.catalogSyncEnabled??false} setValue={value=>void setCatalogSync(value)}/><div className="path"><HardDrive/> {catalogStatus?.syncing?'Synchronisation en cours…':catalogStatus?.sources.find(source=>source.source==='imdb')?.phase??'Catalogue IMDb non synchronisé'}</div>{catalogStatus?.syncing&&<small>{(catalogStatus.sources.find(source=>source.source==='imdb')?.processed??0).toLocaleString('fr-FR')} lignes traitées</small>}<button className="secondary" onClick={()=>void syncCatalog()} disabled={catalogStatus?.syncing||!catalogStatus?.available}><RefreshCw className={catalogStatus?.syncing?'spin':''}/>{catalogStatus?.syncing?'Synchronisation…':'Synchroniser maintenant'}</button><small>IMDb fournit l’inventaire, les notes et la structure des épisodes. La mise à jour complète peut prendre du temps et plusieurs Go lors du premier import.</small></div>
