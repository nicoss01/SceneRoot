...
function LibraryView({heading=true}:{heading?:boolean}) {
  ...
  return <>{heading&&<div className="library-heading"><div className="welcome"><h1>Ma médiathèque</h1><p>{library.items.length} titre{library.items.length>1?'s':''} indexé{library.items.length>1?'s':''} · {unresolved.length} à identifier</p></div><div><button className="secondary" onClick={()=>void scan()} disabled={scanning}><RefreshCw className={scanning?'spin':''}/>{scanning?'Analyse…':'Analyser'}</button>{unresolved.length>0&&<button className="primary" onClick={()=>setMatchingId(unresolved[0].id)}><Search/>Identifier les médias</button>}</div></div>}
    ...
    {library.items.length>0&&!shown.length&&<div className="library-empty"><FolderOpen/><h2>Aucun titre dans ce filtre</h2><p>{kindFilter==='film'?'Votre médiathèque ne contient aucun film indexé.':'Votre médiathèque ne contient aucune série indexée.'}</p><button className="secondary" onClick={()=>setKindFilter('all')}>Tout afficher</button></div>}
...
function PlayerPage({profile}:{profile:Profile}) {
  ...
  useEffect(()=>{
    ...
    const timer=setInterval(()=>{
      ...
      // L'épisode quitté est marqué terminé : advance() enregistre la position
      // courante, on la porte donc à la durée plutôt que d'écrire en parallèle.
...
type KeyState={configured:boolean;source:'environment'|'settings'|'none';maskedKey?:string};
type AppSettings={minFreeGb:number;preferredQuality:string;preferredAudio:AudioChoice;preferredLanguages:string[];preferHdr:boolean;catalogSyncEnabled:boolean;tmdb:KeyState;omdb:KeyState;envSources:{id:string;name:string}[];sources:MaskedSource[]};
type CatalogStatus={available:boolean;total:number;rated:number;syncing:boolean;sources:Array<{source:string;status:string;phase:string;processed:number;startedAt?:string;updatedAt?:string;completedAt?:string;error?:string}>;progress?:{step:number;steps:number;ratio:number};databaseBytes:number;lastSyncAt?:string;nextSyncAt?:string;intervalHours:number;tmdb:{configured:boolean;source:string}};
...
/** « 2 min », « 1 h 12 » : durée écoulée depuis un horodatage ISO. */
...
function SettingsPage() {
  ...
  const saveOmdb=async(remove=false)=>{setSavingCatalog(true);setCatalogMessage('');try{const response=await fetch('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({omdbApiKey:remove?null:omdbKey})});const payload=await response.json().catch(()=>({})) as {error?:string};if(!response.ok)throw new Error(payload.error??'Enregistrement impossible');setOmdbKey('');setCatalogMessage(remove?'Clé OMDb supprimée.':'Clé OMDb validée et enregistrée côté serveur.');await Promise.all([loadSettings(),loadCatalogStatus()])}catch(cause){setCatalogMessage((cause as Error).message)}finally{setSavingCatalog(false)}};
  ...
  return <><div className="welcome"><h1>Paramètres</h1><p>Configurez votre médiathèque, la lecture et l’appareil.</p></div><div className="settings-grid">
    ...
    <div className="settings-card catalog-settings"><h2><Film/>Affiches et résumés OMDb</h2><p>{settings?.omdb.configured?<>Clé configurée via <b>{settings.omdb.source==='environment'?'l’environnement':'SceneRoot'}</b> · {settings.omdb.maskedKey}</>:<>Aucune clé : les affiches viennent de TMDB ou de Wikipédia.</>}</p><div className="source-form"><input type="password" autoComplete="off" placeholder="Clé API OMDb" value={omdbKey} disabled={settings?.omdb.source==='environment'} onChange={event=>setOmdbKey(event.target.value)}/></div><button className="primary" onClick={()=>void saveOmdb()} disabled={savingCatalog||!omdbKey.trim()||settings?.omdb.source==='environment'}><Check/>{savingCatalog?'Validation…':'Valider la clé OMDb'}</button>{settings?.omdb.source==='settings'&&<button className="secondary" onClick={()=>void saveOmdb(true)} disabled={savingCatalog}><Trash2/>Supprimer la clé</button>}<small>OMDb complète ce que TMDB ne fournit pas : affiche, résumé et classification, à partir de l’identifiant IMDb du catalogue local. Les images récupérées passent par le cache de SceneRoot.</small></div>
