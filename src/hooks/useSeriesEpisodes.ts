...
export type EpisodeMeta = { id: string; season: number; episode: number; title: string; overview?: string; still?: string; versions: number; progress: number; position: number };
...
export function useSeriesEpisodes(id: string | undefined, profileId: string, enabled: boolean, seriesTitle: string, art?: string) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/library/group/${encodeURIComponent(id)}/episodes?profileId=${encodeURIComponent(profileId)}`)
      ...
      .then(result => {
        ...
        const playable: MediaItem[] = value.seasons.flatMap(season => season.episodes.map(episode => ({
          id: episode.id,
