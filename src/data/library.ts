...
function quality(group: LibraryGroup): MediaItem['quality'] {
  const height = Math.max(0, ...group.versions.map(version => Number(version.technical?.height ?? 0)));
...
export function libraryGroupToMedia(group: LibraryGroup, index: number, progress?: number): MediaItem {
  return {
    id: group.id,
    title: group.metadata?.title ?? group.title,
    kind: group.kind,
    year: group.year ?? new Date().getFullYear(),
    genres: group.metadata?.genres?.length ? group.metadata.genres : ['Non classé'],
    duration: duration(group),
    rating: 0,
    quality: quality(group),
    description: group.metadata?.overview || 'Média indexé dans votre bibliothèque SceneRoot.',
    palette: palettes[index % palettes.length],
    symbol: group.kind === 'serie' ? '▥' : '◉',
    art: group.metadata?.backdrop ?? group.metadata?.poster,
    source: group.metadata?.provider ?? 'local',
    sourceUrl: group.metadata?.sourceUrl,
    versionCount: group.versions.length,
    episodeCount: group.episodeCount,
    sizeBytes: group.totalSize,
    matched: Boolean(group.metadata),
    local: true,
